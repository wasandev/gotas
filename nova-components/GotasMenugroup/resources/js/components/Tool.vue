    ...
    <div class="flex flex-wrap -mx-3">
      <div class="px-3 mb-6" :class="width" v-for="section in sections" v-if="config.width">
        <loading-card :loading="false" class="px-6 py-4">
          <h3 class="flex mb-3 text-base text-80 font-bold">{{ section.name }}</h3>

          <div class="overflow-hidden overflow-x-auto">
            <ul class="list-reset">
              <li class="text-lg leading-normal pb-1" v-for="resource in section.resources">
                <router-link
                  :to="{ name: 'index', params: { resourceName: resource.uriKey }}"
                  class="no-underline text-primary dim"
...
export default {
  ...
  methods: {
    getToolConfig() {
      Nova.request()
        .get("/nova-vendor/gotas-menugroup")
        .then(({ data }) => {
